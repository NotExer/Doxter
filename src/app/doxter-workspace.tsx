...
export default function DoxterWorkspace() {
          ...
          {file ? (
            ...
            <>
              <span className="upload-icon">
                <Icons.UploadCloud size={25} />
              </span>
              <div>
                <strong>Arrastra tu Excel aquí</strong>
                <p>o selecciónalo desde tu equipo · solo .xlsx</p>
              </div>
              <Icons.ArrowUpRight className="drop-arrow" size={19} />
            </>
          )}
        </div>
        <div className="privacy-note">
          <Icons.Info size={15} /> Los datos se procesan en esta sesión y no se
          almacenan.
        </div>
      </section>
        ...
        >
          <div className="section-heading">
            ...
            <span className="success-chip">
              <Icons.Check size={14} /> Archivo listo
            </span>
          </div>
          <div className="preview-grid">
            ...
            <div className="extract-list">
              ...
              <span>
                <Icons.Building2 size={15} /> Nombre y estado de la empresa
              </span>
              <span>
                <Icons.Activity size={15} /> Actividad económica y cámara
              </span>
              <span>
                <Icons.FileText size={15} /> Categoría y último año renovado
              </span>
            </div>
          </div>
          <button className="primary-button" onClick={handleProcess}>
            <Icons.Play size={17} /> Consultar {nits.length} empresa
            {nits.length === 1 ? "" : "s"}
          </button>
        </section>
      )}
      {processing && (
        <section className="progress-card animate-slide-up" aria-live="polite">
          <div className="progress-top">
            <span>
              <Icons.Loader2 size={17} className="spin" /> Consultando RUES…
            </span>
        ...
        >
          ...
          <div className="result-summary">
            ...
            <button className="text-button" onClick={clearFile}>
              <Icons.Trash2 size={15} /> Nueva consulta
            </button>
          </div>
                    ...
                    <td>
                      {row.error ? (
                        <span className="error-text">
                          <Icons.AlertCircle size={14} /> {row.error}
                        </span>
                      ) : (
                        <strong>
